...
*   **Visibility Gaps** — every `required_data_sources` entry ([Playbook Architecture §7](../04-Playbooks/playbook_architecture.md#7-execution-by-any-executor)) unavailable during triage, each paired with the check it prevented; write "None." when no gap occurred. The countable source for [Visibility-Gap Rate](../05-Metrics/operational_metrics.md); a non-empty entry triggers the `confidence_id` cap of [Playbook Architecture §7](../04-Playbooks/playbook_architecture.md#7-execution-by-any-executor).
...
*   **Visibility Gaps** — every `required_data_sources` entry ([Playbook Architecture §7](../04-Playbooks/playbook_architecture.md#7-execution-by-any-executor)) unavailable during investigation, each paired with the check it prevented; write "None." when no gap occurred. The countable source for [Visibility-Gap Rate](../05-Metrics/operational_metrics.md); a non-empty entry triggers the `confidence_id` cap of [Playbook Architecture §7](../04-Playbooks/playbook_architecture.md#7-execution-by-any-executor).
*   **Provenance** — the playbook(s) used (path plus its `last_updated` date as version), the executor (human / automation / agent — list all classes that materially contributed, e.g. `automation + agent`), the capability classes invoked, and the case/incident ID(s). Required for Glass Box auditability ([Agentic Guardrails](../07-Governance/agentic_guardrails.md)).
