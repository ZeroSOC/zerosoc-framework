...
graph TD
    ...
    Playbooks --> HumanOwner{Human ownership required?}
    HumanOwner -->|"Yes: Crown Jewel, privileged identity, novel behavior, contradiction, timebox"| Handover[Hand over ownership to a human]
    Handover --> Resolve
    HumanOwner -->|No| Resolve{Resolve: score both hypotheses}
    Resolve -->|"Benign proven, or duplicate of an open Case"| CloseI[Close the Case: False Positive / Benign / Duplicate]
    Resolve -->|"Insufficient data"| Playbooks
...
**Low (1) · Medium (2) · High (3)**. The likelihood that the Malicious hypothesis is true. At triage it is the confidence the tagged Findings would yield under the resolution rule of §2.4; Investigation refines it. Sets what may be acted on autonomously.
...
Triage ends in a decision. The permitted outcomes — **Close** the Case (False Positive, Benign, or Duplicate) or **Promote** it to Investigation — are defined in the **Inputs & Outputs** contract at the top of this section; this subsection is the method for choosing between them. There is no "escalate" outcome at triage: stakeholder notification (§3.2) happens only after Investigation confirms an Incident, and a change of Case ownership (§2.3) is a within-phase control, not a triage outcome.

*   **Promote to Investigation** when the observed activity cannot be confidently explained as benign, or when context is insufficient to close.
*   **Close** only when the activity is established as benign, a known false positive, or a duplicate of an open Case. A False Positive emits the tuning signal to Phase 1 (per the contract).
*   **Checklist — can I explain *why* this is suspicious?** If you cannot articulate, in one sentence and with evidence, why the activity is suspicious, you are missing context: gather more. Do not promote out of fear, and do not close prematurely to clear the queue. When context remains genuinely indeterminate, promote — Investigation, not Triage, resolves ambiguity.
...
*   **Summary** — the alert, host, identity, and the core behavior that triggered it. The Summary and the Findings populate the Triage → Investigation phase transition contract, whose fields are listed once in [Playbook Architecture §5](../04-Playbooks/playbook_architecture.md#5-phase-transition-contracts-ocsf-aligned).
*   **Actions Taken** — the logs and systems reviewed, the enrichment capabilities queried, and the internal records consulted.
*   **Findings** — entity context; entity and process analysis; threat-intelligence results; and the historical baseline (was this seen before, and how many of the prior Cases on the same entities or alert type were confirmed True Positive?). Every Finding that bears on a hypothesis carries an **evidence-weight tag** in the vocabulary of §2.4 (`Supports Malicious`, `Strongly supports Benign`, `Contradicts Malicious`, …); Findings that are pure context (asset role, user department) carry none. Tagged Findings are evidence: they seed the Investigation score and are verified, not re-collected, after promotion.
...
*   **Visibility Gaps** — every `required_data_sources` entry ([Playbook Architecture §7](../04-Playbooks/playbook_architecture.md#7-dynamic-agentic-execution)) unavailable during triage, each paired with the check it prevented; write "None." when no gap occurred. The countable source for [Visibility-Gap Rate](../05-Metrics/operational_metrics.md); a non-empty entry triggers the `confidence_id` cap of [Playbook Architecture §7](../04-Playbooks/playbook_architecture.md#7-dynamic-agentic-execution).
...
There are always exactly two sides. Competing explanations within a side — which Incident Category on the malicious side, which authorized activity on the benign side — are sub-hypotheses; the evidence they gather scores for their side (§2.4).

### 2.2 Parallel Playbook Execution
1. Verify the tagged Findings of the Triage Note (§1.6): confirm each still holds and keep its evidence-weight tag; the score starts from them. Then apply the **domain → Incident Category pivot**: using the candidate `incident_category` from the Triage → Investigation phase transition contract, open the matching Investigation & Response playbook in [`04-Playbooks/02-Investigation-Response/`](../04-Playbooks/02-Investigation-Response/); if no specialized playbook exists, use the catch-all `00-generic.md`. The technique lists in the playbooks and alert types are indicative, not exhaustive: the relation between techniques and Incident Categories is intentionally loose, and a category is decided by the adversary's objective and impact, not by technique lookup. (Contract fields per [Playbook Architecture §5](../04-Playbooks/playbook_architecture.md).)
2. Execute the validation queries defined in the playbook. Queries must be **discriminating**: a result should support one hypothesis and contradict the other. Seek evidence that could refute the favoured hypothesis, not only evidence that confirms it — that is the guard against confirmation bias.
3. Resolve the hypotheses per §2.4, then act on the outcome: a proven Malicious hypothesis promotes the Case to a confirmed Incident (§3.1); a proven Benign hypothesis closes the Case (a False Positive submits a tuning ticket to [Phase 1: Preparation & Engineering](01-preparation_and_engineering.md)); contradiction or insufficient data follow §2.3 and §2.4.
4. **Re-classify on evidence drift:** if evidence reveals the activity's objective differs from, or exceeds, the current candidate Incident Category (e.g. a commodity loader staging ransomware, or phishing that yielded credentials), update the candidate `incident_category`, switch to the matching Investigation & Response playbook, and carry all accumulated evidence forward. Record the pivot, and its reason, in the Investigation Note (§2.5). The classification remains provisional until `verdict_id = 2` is set at incident promotion (§3.1).

### 2.3 Conditions Requiring Human Ownership (Handover)
Every Case has one **owner** (OCSF `assignee`): the executor responsible for advancing it and for its verdict. Whatever executor holds a Case, a **human must own it** when any of the following conditions holds. If the owner is automation or an agent, it hands the Case over to a human ([handover](../01-Foundation/definitions.md#handover)); a human owner simply continues.
*   **Crown Jewel or privileged identity:** the affected asset is classified as a Crown Jewel, or the affected identity is highly privileged (e.g., Domain Admin, Global/Cloud Administrator).
*   **Novel behavior:** the observed activity is not covered by the validation queries of any applicable Investigation & Response playbook, the generic one included — an unknown exploit, or behavior the playbooks do not address.
*   **Contradiction:** the validation queries yield contradictory evidence that leaves the verdict indeterminate (§2.4).
*   **Indeterminacy past the timebox (automation and agents):** neither hypothesis reaches the §2.4 resolution bar within the investigation timebox — **10 minutes** for High/Critical severity, **20 minutes** otherwise — or the Case token budget is exhausted. The Case is handed over as **indeterminate**; the human may close it (verdict `Insufficient Data`) with a monitoring watch, extend the investigation, or take it over. Timebox and budget enforcement is specified in [Agentic Guardrails §5](../07-Governance/agentic_guardrails.md#5-resource--token-metering).

A handover transfers **ownership of the Case and of its verdict**. It does not block containment actions that [Incident Response §2.1](03-response.md#21-risk-based-autonomy-matrix-for-containment) pre-authorizes and [Agentic Guardrails §3](../07-Governance/agentic_guardrails.md#3-autonomous-action-boundaries) bounds: if the playbook detects an imminent threat of impact escalation (e.g., active ransomware propagation, credential abuse, or data exfiltration), the automation or agent executes the pre-authorized, reversible containment actions and notifies the new human owner concurrently.

### 2.4 Hypothesis Resolution, Verdict and Confidence

Every finding — a tagged Triage Note Finding (§1.6) or a validation query result — carries one of three **evidence weights**, expressed as six directional tags. Playbook query tags MUST use exactly this vocabulary — `Supports Malicious` / `Supports Benign` / `Strongly supports Malicious` / `Strongly supports Benign` / `Contradicts Malicious` / `Contradicts Benign`:

*   **Strongly supports** (score 2) — a finding that, unrebutted, is sufficient on its own to prove the hypothesis (e.g. multi-engine hash consensus on a known malware family).
*   **Supports** (score 1) — corroborating evidence; not conclusive on its own.
*   **Contradicts** — evidence against the hypothesis; an unexplained contradiction blocks that hypothesis, whatever its score.

**Score.** Each hypothesis scores the sum of its findings, triage and investigation together: 2 per strongly-supporting finding, 1 per supporting finding. Findings derived from the same underlying telemetry artifact count once — independence matters, and a triage lookup and an investigation lookup on the same indicator are one finding. A Case may arrive from triage already scoring enough on one side; the playbook's discriminating queries still run, because a verdict requires that the other side was tested and that no unexplained contradiction remains. A hypothesis is **proven** when its score is 2 or more and no unexplained contradicting finding remains; it is **blocked** while an unexplained contradicting finding remains.

**Verdict and confidence.** The two hypotheses resolve as follows:

| Outcome | Verdict (`verdict_id`) | Confidence (`confidence_id`) | Next step |
|---|---|---|---|
| Only the Malicious hypothesis is proven | True Positive (`2`) | High (`3`) if at least one strongly-supporting finding; otherwise Medium (`2`) | Promote to Incident (§3.1) |
| Only the Benign hypothesis is proven | False Positive (`1`) if the detection was wrong; Benign Positive (`5`) if the activity was authorized | same rule | Close; a False Positive emits a tuning ticket to Phase 1 |
...
| Both proven, or both blocked | none — **contradictory** | — | Human ownership (§2.3); the human resolves the contradiction or closes as Insufficient Data (`7`) |
| Neither proven | none — **insufficient data** | — | Run the playbook's remaining queries or add discriminating ones; past the timebox or budget, human ownership (§2.3) |

Autonomous containment requires High confidence; at Medium confidence, prefer human-in-the-loop review per [Agentic Guardrails](../07-Governance/agentic_guardrails.md). Visibility gaps cap confidence as specified in [Playbook Architecture §7](../04-Playbooks/playbook_architecture.md#7-dynamic-agentic-execution).

This generalizes what playbooks state per Incident Category (e.g. the query-confluence note in the [Commodity Malware playbook](../04-Playbooks/02-Investigation-Response/05-commodity_malware.md), where a strong hash consensus alone, or two converging delivery-chain findings together, are each sufficient to declare the incident).
...
*   **Executed Queries** — the validation queries run, each with its outcome and evidence-weight tag (§2.4).
*   **Resolution Rationale** — the §2.4 resolution applied: which findings proved or disproved Malicious hypothesis or B, and why (strongly-supporting finding, or which findings converge).
...
*   **Visibility Gaps** — every `required_data_sources` entry ([Playbook Architecture §7](../04-Playbooks/playbook_architecture.md#7-dynamic-agentic-execution)) unavailable during investigation, each paired with the check it prevented; write "None." when no gap occurred. The countable source for [Visibility-Gap Rate](../05-Metrics/operational_metrics.md); a non-empty entry triggers the `confidence_id` cap of [Playbook Architecture §7](../04-Playbooks/playbook_architecture.md#7-dynamic-agentic-execution).
*   **Provenance** — the playbook(s) used (path plus its `last_updated` date as version), the executor (human / automation / agent — list all classes that materially contributed, e.g. `automation + agent`), the capability classes invoked, and the case/incident ID(s). Required for Glass Box auditability ([Agentic Guardrails](../07-Governance/agentic_guardrails.md)).
