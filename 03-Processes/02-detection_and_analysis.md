...
**Low (1) · Medium (2) · High (3)**. The likelihood that the Malicious hypothesis is true, set by hypothesis resolution (§2.4). Sets what may be acted on autonomously.
...
*   **Findings** — entity context; entity and process analysis; threat-intelligence results; and the historical baseline (was this seen before, and how many of the prior Cases on the same entities or alert type were confirmed True Positive?).
...
| | |
|---|---|
| **Consumes** | The promoted Case, its Triage → Investigation phase transition contract, and its Triage Note (§1.6). The Note's Findings — the enrichment of §1.2 and the scope and correlation results of §1.3 — are the starting evidence of the investigation; they are not repeated, they are extended. |
...
Every validation query result is tagged with one of three **evidence weights**, expressed as six directional tags. Playbook query tags MUST use exactly this vocabulary — `Supports Malicious` / `Supports Benign` / `Strongly supports Malicious` / `Strongly supports Benign` / `Contradicts Malicious` / `Contradicts Benign`:
...
**Score.** Each hypothesis scores the sum of its findings: 2 per strongly-supporting finding, 1 per supporting finding. Findings derived from the same underlying telemetry artifact count once — independence matters. A hypothesis is **proven** when its score is 2 or more and no unexplained contradicting finding remains; it is **blocked** while an unexplained contradicting finding remains.
