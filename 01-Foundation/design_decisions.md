...
## DD-24: Response Actions Are Their Own Note Element

- **Decision:** A Triage Note and an Investigation Note render **Response Actions** as an element of their own. *Response Actions* is the framework's own term, and deliberately not an OCSF one, because it groups two things OCSF keeps apart: the **observed remediation activity**, which maps to the [Remediation Activity [7001]](https://schema.ocsf.io/1.9.0/classes/remediation_activity) events the Case references, and the **suggested or planned remediation actions**, which map to **tickets** (internal or external) the Case raised. Every ticket the Case raises says which kind it is, e.g.: `tuning`, `visibility`, `containment`, `eradication` or `recovery`. Implemented in [Detection & Analysis §1.6 and §2.5](../03-Processes/02-detection_and_analysis.md) and the [Case Schema](../02-Taxonomy/case_schema.md).
- **Rationale:** A reader of a Note needs one place that answers what was done and what is about to be done, and all the remediation — observed or planned — needs to be tracked consistently. The Case could not tell Response what it was being handed. Planned actions are tickets because in OCSF a Remediation Activity records an attempt that was made, with a required time and an outcome, so it cannot stand for an action still waiting for approval. A ticket can, and a kind on the ticket is what tells a tuning request from a containment step. The element takes a framework name rather than the OCSF class name because it holds more than the class does: naming it for the class would say that a planned action is a Remediation Activity, which is exactly what OCSF cannot express.
- **Discarded alternatives:** render actions only among the Observations, as before (an action waiting for approval has nowhere to go, so a Note cannot say what Response is being handed); name the element **Remediation Activity** after the OCSF class (borrows an OCSF name for something wider than the class, and a reader would expect every entry to be one); give a Remediation Activity a planned state (its status values are outcomes of an attempt, not states before one); one kind for all remediation instead of three (the Phase 3 stages are what a reader acts on, and one word for all three hides which stage a Case is in).
- **Revisit:** when OCSF can represent a remediation that has not been attempted, which retires the ticket mapping for planned actions and could collapse the group into one class.

## DD-25: The Framework's Entry Is an Observation, Not a Finding

- **Decision:** One entry in a Case's record is an **Observation**: a tagged assertion carrying a side and a confidence, or neither where it is pure context. *Finding* is left to OCSF, where it names a class of event. The relationship is one chain — a Case ([Incident Finding [2005]](https://schema.ocsf.io/1.9.0/classes/incident_finding)) holds Observations (`finding_info[]`), and each Observation's `related_events[]` cite what it rests on: an **Alert** ([Detection Finding [2004]](https://schema.ocsf.io/1.9.0/classes/detection_finding)), an observed **remediation** ([Remediation Activity [7001]](https://schema.ocsf.io/1.9.0/classes/remediation_activity)), or an **event** of any class in the System Activity, Application Activity or Network Activity categories. `types` names which of the three an entry is: `alert`, `event` or `action`. Defined in [Definitions](definitions.md#observation), mapped in the [Case Schema](../02-Taxonomy/case_schema.md).
- **Rationale:** One word carried two meanings, and it was the most load-bearing word in hypothesis resolution. In OCSF a Detection Finding is this framework's Alert and an Incident Finding is its Case, while the framework's Finding was finer than either — one entry in the record, of which an Alert is one kind. The collision was documented rather than resolved, and it cost: the side and the confidence went into `tags`, whose OCSF purpose is to categorise and search, because an object describing a finding has no field for an entry's verdict; response actions went into a list of findings because there was nowhere else to put them; and `analytic` carried what selected an action because it was the only field to hand. Naming the framework's own concept removes the cause instead of the symptoms. `finding_info` stays the carrier because OCSF offers no other: `finding_info_list` is required on Incident Finding and is the only link between a Case and what it aggregates.
- **Discarded alternatives:** keep the collision and warn about it in the glossary (what the framework did, and the warning did not prevent any of the three stretches above); rename OCSF's sense instead (not the framework's to rename, and OCSF is the dependency, not the dependent).
