...
| `related_events` | The **evidence**: the events the Finding rests on — below |
...
**Evidence.** A Finding's evidence is its `related_events`: the events it rests on. This is the one place evidence is held — there is no second list beside the Findings, and a raw log is never a Finding's body.

The classes a Case's Findings cite are not only Alerts and Cases: a Detection Finding where the Finding is an Alert, a Remediation Activity on an `action` entry, the events of the telemetry a validation query returned. An event OCSF does not carry is named by `type` instead, which `related_event` exists to allow.
...
| The evidence each Finding rests on | the events it rests on, in its `related_events` (§3) |
